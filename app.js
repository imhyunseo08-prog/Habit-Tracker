...
useEffect(() => {
  ...
  // 2) Firestore 저장
  const user = window.currentUser; // index.html onAuthStateChanged에서 넣어둔 값
  if (!user || !window.db || !window.fs) return;

  window.fs.setDoc(
    window.fs.doc(window.db, "users", user.uid),
    { items, checks, themes, updatedAt: Date.now() },
    { merge: true }
